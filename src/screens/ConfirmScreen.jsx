...
const ConfirmScreen = () => {
  ...
  useEffect(() => {
    ...
    if (cart && cart.length > 0) {
      ...
      // If no cart, redirect away to avoid empty confirm state
      if (!placedOrder) {
    ...
    >
      ...
      >
        ...
        <p className="text-text-secondary text-[15px] max-w-[280px] mx-auto leading-relaxed mb-10">
          Success! Your order <span className="font-mono text-primary font-bold">{orderId}</span> has being processed with priority.
        </p>

        <div className="bg-surface border border-border/60 rounded-[28px] p-6 mb-10 shadow-[0_4px_25px_rgba(0,0,0,0.03)] w-full max-w-[340px] text-left">
        ...
        <button 
          ...
          className="w-full max-w-[280px] bg-[#1A1A1A] text-white h-[60px] rounded-2xl font-bold shadow-[0_10px_25px_rgba(0,0,0,0.15)] active:scale-95 transition-all flex items-center justify-center space-x-3 tracking-wide"
        >
