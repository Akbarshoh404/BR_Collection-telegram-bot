          ...
          userOrders.map(order => (
            <div key={order.id} className="bg-surface border border-border/60 rounded-2xl p-5 shadow-[0_4px_20px_rgba(0,0,0,0.03)] relative overflow-hidden">
                ...
                {order.items.map((item, i) => (
                  <div key={i} className="flex justify-between items-center">
                    <div className="flex items-center space-x-3 truncate">
                      <div className="w-10 h-10 bg-muted/10 rounded-lg flex items-center justify-center text-xl">
                        {item.product.emoji}
                      </div>
              ...
              <div className="border-t border-border/50 mt-4 pt-4 flex justify-between items-center">
                ...
                <span className="text-base font-bold text-accent-gold">{formatPrice(order.items.reduce((sum, item) => sum + item.finalPrice, 0))}</span>
              </div>
            </div>
          ))
        )}
