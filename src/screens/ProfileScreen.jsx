    ...
    >
      ...
      <header className="pt-6 pb-6 px-6 bg-surface border-b border-border/50 shadow-[0_4px_20px_rgba(0,0,0,0.02)] relative overflow-hidden">
        ...
        <div className="flex items-center space-x-5 relative z-10">
          <motion.button 
            whileTap={{ scale: 0.95 }}
            className="w-[84px] h-[84px] bg-[#1A1A1A] text-white rounded-full flex items-center justify-center text-3xl font-bold shadow-[0_8px_20px_rgba(0,0,0,0.15)] relative cursor-pointer active:scale-95 transition-transform border-[3px] border-surface overflow-hidden"
          >
        ...
        <div className="grid grid-cols-2 gap-4 mt-10 relative z-10 px-1">
          <motion.div 
            ...
            className="bg-surface rounded-2xl p-5 border border-border/60 shadow-[0_4px_15px_rgba(0,0,0,0.03)] flex flex-col items-center"
          >
      ...
      <div className="p-5 space-y-7 mt-2">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          ...
          <div className="bg-surface rounded-[24px] border border-border/50 shadow-[0_4px_20px_rgba(0,0,0,0.02)] overflow-hidden">
            <motion.button 
              ...
              whileHover={{ backgroundColor: 'rgba(192, 187, 181, 0.1)' }}
              whileTap={{ scale: 0.98 }}
              className="w-full flex items-center justify-between p-4 px-5 bg-transparent transition-colors border-b border-border/50"
            >
            ...
            <motion.button 
              ...
              whileHover={{ backgroundColor: 'rgba(192, 187, 181, 0.1)' }}
              whileTap={{ scale: 0.98 }}
              className="w-full flex items-center justify-between p-4 px-5 bg-transparent transition-colors"
            >
              <div className="flex items-center space-x-3 text-primary">
        ...
        {!currentUser?.isAdmin && (
          <motion.button
            ...
            className="w-full bg-muted/10 text-text-secondary p-4 rounded-[20px] font-bold text-[14px] transition-all hover:bg-muted/20 border border-border/50"
          >
        ...
        {showAdminLogin && (
          <div className="fixed inset-0 z-50 flex items-end justify-center">
            ...
            <motion.div 
              ...
              className="relative w-full max-w-[430px] bg-background rounded-t-[32px] p-6 pb-safe shadow-[0_-10px_40px_rgba(0,0,0,0.2)] border-t border-border/50 min-h-[50vh]"
            >
              <div className="flex justify-between items-center mb-6 pt-2">
                ...
                <motion.button 
                  onClick={() => setShowAdminLogin(false)} 
                  whileTap={{ scale: 0.9 }}
                  className="w-9 h-9 flex items-center justify-center bg-muted/20 text-[#1A1A1A] rounded-full hover:bg-muted/40 transition-colors shadow-sm"
                >
              ...
              <div className="bg-muted/5 border border-border/40 p-5 rounded-2xl mb-8">
                <div className="flex items-start space-x-4">
                  ...
                  <p className="text-[13px] font-medium text-text-secondary leading-relaxed">
                    Administrative access allows inventory management and order oversight. Default: <span className="text-primary font-bold">admin / 1234</span>
                  </p>
                </div>
              </div>
              ...
              <form onSubmit={handleAdminLogin} className="space-y-4">
                <div>
                  ...
                  <input 
                    ...
                    className={`w-full border-2 ${loginError ? 'border-error/50 bg-error/5' : 'border-border/50 bg-surface'} rounded-2xl px-4 py-4 focus:border-accent-gold focus:outline-none text-[15px] font-bold text-[#1A1A1A] shadow-inner transition-colors`} 
                ...
                <div>
                  ...
                  <input 
                    ...
                    className={`w-full border-2 ${loginError ? 'border-error/50 bg-error/5 text-error' : 'border-border/50 bg-surface text-[#1A1A1A]'} rounded-2xl px-4 py-4 focus:border-accent-gold focus:outline-none text-[15px] font-bold shadow-inner tracking-widest font-mono transition-colors`} 
                    placeholder="••••" 
                    required 
                  />
                </div>
                ...
                {loginError && (
                  <motion.div 
                    ...
                    animate={{ opacity: 1, y: 0 }} 
                    className="pl-1 pt-1 flex items-center space-x-2"
                  >
                ...
                <div className="pt-6 pb-2">
                  <motion.button 
                    type="submit" 
                    whileTap={{ scale: 0.95 }}
                    className="w-full py-4 h-[56px] rounded-2xl font-bold bg-[#1A1A1A] text-white shadow-[0_8px_20px_rgba(0,0,0,0.15)] active:scale-95 transition-transform text-[15px] tracking-wide relative overflow-hidden group hover:bg-black"
                  >
